# Factoring the Age_Bin and Income_Bin
cdata <- cdata %>%
  mutate(Age_Bin = recode(Age_Bin, 
                             `2` = "Young Adults", 
                             `3` = "Adults",
                             `4` = "Middle Age", 
                             `5` = "Seniors"))
cdata <- cdata %>%
  mutate(Income_Bin = recode(Income_Bin, 
                             `1` = "Lower Income (Working)", 
                             `2` = "Lower-Middle Income (Stable)",
                             `3` = "Middle Income (Comfortable)", 
                             `4` = "Upper-Middle Income (Wealthy)",
                             `5` = "High Income (Elite"))
View(cdata)
...
cdata.cluster.income <- cdata %>%
  group_by(Cluster, Income_Bin) %>%
  summarise(Customer_Count = n(), .groups = "drop")

# Plot Customer Count based on Cluster and Income Level
ggplot(cdata.cluster.income, 
       aes(x = as.factor(Cluster), y = Customer_Count, 
           fill = as.factor(Income_Bin))) +
  geom_bar(stat = "identity", position = "dodge") + 
